...
char getNumeral(int ); 
//UPDATING THE VALUE OF PROGRAM COUNTER
...
int getNumeralValue(char num) //CONVERTING THE HEXADECIMAL FROM TO DECIMAL FORM
{ 
	if( num >= '0' && num <= '9') 
		return (num - '0'); 
	if( num >= 'A' && num <= 'D') 
...
char getNumeral(int val) //CONVERTING THE DECIMAL VALUE TO HEXADECIMAL FORM
{ 
	if( val >= 0 && val <= 9) 
		return (val + '0'); 
	if( val >= 10 && val <= 14) 
